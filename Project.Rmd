...
mets <- compute_metrics(output, series_GDP, N_train, k=4)
...
model {
  ...
  for (t in (order+1):N_train) {
    mu[1:2, t, 1] <- m0
    for (j in 1:order) {
      mu[1:2,t,j+1] <- mu[1:2, t, j] + A[,,j] %*% y[,(t-j)]
    }
    y[1:2,t]     ~ dmnorm(mu[1:2,t,order+1], tau)
    yp_in[1:2,t] ~ dmnorm(mu[1:2,t,order+1], tau)
